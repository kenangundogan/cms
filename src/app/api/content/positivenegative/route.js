// src/app/api/content/positivenegative/route.js
...
export async function GET(req) {
    ...
    const createPaginationLinks = (currentPage, totalPages) => {
        ...
        // Previous link
        links.push({
            url: currentPage > 1 ? `/api/content/positivenegative?page=${currentPage - 1}` : null,
            label: "Previous",
            active: false,
        });
        ...
        if (startPage > 1) {
            links.push({
                url: `/api/content/positivenegative?page=1`,
                label: "1",
        ...
        for (let i = startPage; i <= endPage; i++) {
            links.push({
                url: `/api/content/positivenegative?page=${i}`,
                label: `${i}`,
        ...
        if (endPage < totalPages) {
            ...
            links.push({
                url: `/api/content/positivenegative?page=${totalPages}`,
                label: `${totalPages}`,
        ...
        // Next link
        links.push({
            url: currentPage < totalPages ? `/api/content/positivenegative?page=${currentPage + 1}` : null,
            label: "Next",
            active: false,
        });
    ...
    return new Response(
        JSON.stringify({
            ...
            meta: {
                ...
                path: "/api/content/positivenegative",
                per_page: perPage,
